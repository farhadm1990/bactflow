...
def gene_counter(gene_files: str, gene_type: str, out_name: str) -> None:
    ...
    header = ["Locus_tag",   "Type",    "Lenght",   "Gene", "EC_number",  "COG", "Product"]

    gene_counts = {}
    gnt = gene_type.split(',') #to allow multiple genes to be passed on comma-seperated

    for i in glob.glob(os.path.join(gene_files, "*/*")):
        if i.endswith(".tsv"):
            df = pd.read_csv(i, sep= '\t', header=None, names = header, skiprows=1)
